...
async def lifespan(app: FastAPI):
    ...
    async def attach(host: str, port: int, tag: str):
        # Separate firehose + fleet connections per broker so sparse `_stats.>`
        # heartbeats are never starved by the `>` delivery flood. The firehose is
        # gated: at giant fanout it starves the observer loop and undercounts, so
        # OBSERVER_FIREHOSE=0 drops it and throughput comes from summed broker OTel.
        clients, tasks = [], []
        if config.OBSERVER_FIREHOSE:
            tp = await config.connect_broker_at(log, host, port, deadline=5.0)
            clients.append(tp)
            tasks.append(asyncio.create_task(_pump_throughput(tp, tag, stats, stop)))
        fl = await config.connect_broker_at(log, host, port, deadline=5.0)
        clients.append(fl)
        tasks.append(asyncio.create_task(_pump_fleet(fl, tag, stats, stop)))
...
app = FastAPI(title="pubsub.py distributed observer", lifespan=lifespan)
