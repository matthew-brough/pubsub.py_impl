"""Load generator: publishes at a target rate through the nginx ingress.

One of X producer replicas. Each connects to the ingress (``PUBSUB_HOST=nginx``),
which round-robins the TCP connection onto one broker, so producer load spreads
across the broker pool. Every message carries ``extras={"seq","pub_ts"}`` so the
observer can measure end-to-end latency. Publish throughput is reported from here
(independent of any subscriber) so it stays trustworthy under saturation.
"""
...
async def run() -> None:
    ...
    log.info(
        "connected to ingress %s:%d; producing mode=%s workers=%d payload=%dB",
        config.PUBSUB_HOST,
        config.PUBSUB_PORT,
